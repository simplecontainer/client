	...
	return command.Command{
		...
		Functions: []func(*manager.Manager, []string){
			func(mgr *manager.Manager, args []string) {
				...
				resp, err := network.Raw(mgr.Context.Client, fmt.Sprintf("%s/api/v1/debug/%s/%s", mgr.Context.ApiURL, format.ToString(), strconv.FormatBool(viper.GetBool("f"))), http.MethodGet, nil)
