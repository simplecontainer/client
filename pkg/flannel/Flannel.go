...
func flannel(ctx context.Context, config *configuration.Configuration, flannelIface *net.Interface, flannelIPv6Masq bool, netMode int) error {
	extIface, err := LookupExtInterface(flannelIface, netMode)
	if err != nil {
		return errors.Wrap(err, "failed to find the interface")
	}

	config.Flannel.InterfaceFlannel = extIface
	...
	prevSubnet := ReadCIDRFromSubnetFile(config.Flannel.ConfigFile, "FLANNEL_SUBNET")
	prevIPv6Subnet := ReadIP6CIDRFromSubnetFile(config.Flannel.ConfigFile, "FLANNEL_IPV6_SUBNET")
	...
	config.Flannel.Network = flannelConfig.Network
	config.Flannel.Networkv6 = flannelConfig.IPv6Network
